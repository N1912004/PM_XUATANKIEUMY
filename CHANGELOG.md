...
- Tối ưu hóa hệ thống màu sắc chủ đạo giao diện (Theme Palette): Đồng bộ tất cả các biến CSS (`--po-bl`, `--bl`, `--sup-bl`), nút bấm "Tạo yêu cầu", "Tạo mới", icon thống kê và thẻ tiêu đề trên tất cả các trang (`LeaveOvertime`, `Employee`, `FoodSafetyAudit`, `Timekeeping`, `PurchaseOrder`, `Menu`, `Supplier`, `Area`, `Recipe`, `ListHang`, `BaoCao`, `ChatNhom`) tự động kết nối động tới cấu hình **Màu chủ đạo** tại Cài đặt Hệ thống (`Setting::get('primary_color')` tại `/admin/system-settings?tab=-thuong-hieu-tab`), loại bỏ triệt để các mã màu cứng `#267DC1`.
- Nâng cấp file xuất Excel thực đơn tuần (`MenuExport.php` ➔ Sheet `TD`) khớp 100% Ảnh mẫu 2: Đa ngôn ngữ hoàn chỉnh toàn bộ ô tiêu đề (`B2` ➔ `SHIFT`/`CA`, `C2` ➔ `DISH`/`MÓN`, `C3` ➔ `DISH 1`/`MÓN 1`...), áp dụng mã màu chuẩn 100% từ file mẫu (`B2/A2/B3` fill xám `#BFBFBF` chữ đỏ, `C2:J2` fill xám nhạt `#D8D8D8` chữ đỏ, `C3` fill kem đào `#FBE4D5` chữ xanh, viền xanh cyan `#00B0F0`), nhúng trực tiếp Logo hệ thống từ Cài đặt (`Setting::get('site_logo')`), tự động lấy tên ca động từ DB (`Shift`) và **tự động ẩn hoàn toàn các khối ca không có thực đơn trong tuần**.
