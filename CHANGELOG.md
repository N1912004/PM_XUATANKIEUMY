...
- Fix lỗi nhận diện nhầm mẫu xuất: Xử lý truyền cờ `$isSingleDay` chính xác dựa theo dải ngày (`from` và `to`), đảm bảo khi bấm nút **Xuất Excel** trên trang Lập thực đơn tuần (`activeView=week`) sẽ luôn tự động tải đúng Mẫu Ma trận Thực đơn Tuần (Sheet `TD`), ngay cả khi tuần đó mới chỉ nhập 1 món ăn.
