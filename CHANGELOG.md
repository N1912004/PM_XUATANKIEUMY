...
- Nâng cấp file xuất Excel thực đơn tuần (`MenuExport.php` ➔ Sheet `TD`) khớp 100% Ảnh mẫu 2: Nhúng màu pastel chuẩn (Tiêu đề cột xanh nhạt `#D9E1F2`, Cột A cam đào `#FCE4D6`, Cột C xám nhạt `#EDEDED`), viền xanh cyan `#00B0F0`, logo BlueFire A1, tiêu đề A1 2 dòng (đỏ đậm 15pt / đỏ nghiêng 12pt), tự động lấy tên ca động từ DB (`Shift`) và **tự động lọc bỏ hoàn toàn các khối ca không có thực đơn trong tuần** (chỉ xuất các ca thực tế có món).
