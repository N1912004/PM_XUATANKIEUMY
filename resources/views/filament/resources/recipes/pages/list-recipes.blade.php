        ...
        @if($recipesList->total() > 0)
            ...
            <div class="tf">
                ...
                <div class="pgwrap">
                    ...
                    <select wire:model.live="perPage" class="lv-per-page-select" style="min-width:68px; height:32px; padding:0 24px 0 10px; font-size:13px; font-weight:400; color:var(--tx, #0f172a); background-color:var(--wh, #ffffff); border:1px solid var(--bd, #cbd5e1); border-radius:8px; outline:none; appearance:none; -webkit-appearance:none; background-image:url(&quot;data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2364748b' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e&quot;); background-position:right 8px center; background-repeat:no-repeat; background-size:16px 16px; cursor:pointer;">
                        @foreach([5, 10, 20, 30, 50] as $count)
                            <option value="{{ $count }}">{{ $count }}</option>
                        @endforeach
                    </select>
                    @if($recipesList->total() > 0)
                        <div class="pgbs" style="display:flex; align-items:center; gap:4px;">
                            {{-- Previous --}}
                            @if ($recipesList->onFirstPage())
                                <button type="button" class="pgb" disabled style="display:inline-flex; align-items:center; justify-content:center; min-width:30px; height:30px; padding:0 6px; border:1px solid var(--bd, #e2e8f0); border-radius:6px; background:transparent; color:var(--mu, #94a3b8); opacity:.4; cursor:not-allowed;">
                                    <i class="fa-solid fa-chevron-left" style="font-size:10px"></i>
                                </button>
                            @else
                                <button type="button" wire:click="previousPage" class="pgb" style="display:inline-flex; align-items:center; justify-content:center; min-width:30px; height:30px; padding:0 6px; border:1px solid var(--bd, #cbd5e1); border-radius:6px; background:var(--wh, #fff); color:var(--mu, #64748b); cursor:pointer;">
                                    <i class="fa-solid fa-chevron-left" style="font-size:10px"></i>
                                </button>
                            @endif
                            ...
                            @foreach ($pageWindow as $i => $page)
                                @if ($i > 0 && $page - $pageWindow[$i - 1] > 1)
                                    <span class="pgdot" style="display:inline-flex; align-items:center; justify-content:center; min-width:20px; height:30px; padding:0 2px; color:var(--mu, #94a3b8);">...</span>
                                @endif
                                @if ($page == $currentPage)
                                    <button type="button" class="pgb cur" style="display:inline-flex; align-items:center; justify-content:center; min-width:30px; height:30px; border-radius:6px; background:var(--bl, #2563eb); color:#ffffff; font-weight:700; font-size:13px; border:none;">{{ $page }}</button>
                                @else
                                    <button type="button" wire:click="gotoPage({{ $page }})" class="pgb" style="display:inline-flex; align-items:center; justify-content:center; min-width:30px; height:30px; padding:0 8px; border:1px solid var(--bd, #cbd5e1); border-radius:6px; background:var(--wh, #fff); color:var(--tx, #0f172a); font-size:13px; font-weight:500; cursor:pointer;">{{ $page }}</button>
                                @endif
                            @endforeach

                            {{-- Next --}}
                            @if ($recipesList->hasMorePages())
                                <button type="button" wire:click="nextPage" class="pgb" style="display:inline-flex; align-items:center; justify-content:center; min-width:30px; height:30px; padding:0 6px; border:1px solid var(--bd, #cbd5e1); border-radius:6px; background:var(--wh, #fff); color:var(--mu, #64748b); cursor:pointer;">
