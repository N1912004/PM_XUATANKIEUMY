    ...
    <div class="tcard">
        ...
        @if($shifts->total() > 0)
            <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-top:14px;padding:14px 16px;border-top:1px solid var(--po-bd2);font-size:12px;color:var(--po-mu)">
                ...
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-size:12px; color:var(--po-mu)">{{ __('common.pagination.per_page_label') }}</span><select wire:model.live="shiftPerPage" style="height:30px;border:1px solid var(--po-bd);border-radius:6px;padding:0 8px;font-size:12px;background:transparent">@foreach([5,10,20,50] as $count)<option value="{{ $count }}">{{ $count }}</option>@endforeach</select>
